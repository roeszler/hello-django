...
class Item(models.Model):
    """
    Class Inheritance: we can inherit the base model class by putting models.Model.
    Our "item" class can now do everything the built-in Django model class can do.
    """
    # define the attributes that our individual items will have:

    # item_id = created automatically by Django

    # built-in Django field to indicate it will just have characters / text in it
